...
function renderTimeline(){
  ...
  state.reservations.forEach(reservation=>{const rawStart=new Date(reservation.start_time),rawEnd=new Date(reservation.end_time);if(rawStart>=dayEnd||rawEnd<=dayStart)return;const start=new Date(Math.max(rawStart,dayStart)),end=new Date(Math.min(rawEnd,dayEnd));const block=document.createElement("div");block.className=`availability-block${reservation.mine?" mine":""}`;block.style.left=`${(start-dayStart)/DAY*100}%`;block.style.width=`${(end-start)/DAY*100}%`;const description=reservation.mine?`Your reservation, ${time(rawStart)} to ${time(rawEnd)}`:`Unavailable, ${time(rawStart)} to ${time(rawEnd)}`;block.title=description;block.setAttribute("aria-label",description);timeline.append(block)});
  ...
  if(state.selection&&!activeMine()&&sameDay(state.selection.start,dayStart)){const block=document.createElement("div");const conflict=selectionError();block.className=`selection-block${conflict?" conflict":""}`;block.style.left=`${(state.selection.start-dayStart)/DAY*100}%`;block.style.width=`${(state.selection.end-state.selection.start)/DAY*100}%`;block.setAttribute("aria-label",`Selected: ${time(state.selection.start)}, ${durationLabel((state.selection.end-state.selection.start)/60000)}`);block.title=`${time(state.selection.start)} · ${durationLabel((state.selection.end-state.selection.start)/60000)}`;timeline.append(block);requestAnimationFrame(()=>{block.classList.add("updated");const scroller=timeline.parentElement,target=block.offsetLeft+block.offsetWidth/2-scroller.clientWidth/2;scroller.classList.remove("reveal-scrollbar");scroller.classList.add("is-auto-scrolling");scroller.scrollTo({left:Math.max(0,target),behavior:"smooth"});clearTimeout(scroller.autoTimer);scroller.autoTimer=setTimeout(()=>scroller.classList.remove("is-auto-scrolling"),500)});setTimeout(()=>block.classList.remove("updated"),220)}
}
...
function selectionError(){if(!state.selection)return"";const{start,end}=state.selection;if(start<=new Date())return"This time has already passed.";if(end>addDays(startOfDay(start),1))return"The session must finish before midnight.";if(overlapReservation(start,end))return"Part of this session overlaps an unavailable time.";if(end-start>3*60*60*1000)return"Reservations can be up to three hours.";return""}
function updateSelection(){
  const{start,end}=state.selection,minutes=Math.round((end-start)/60000),invalidReason=selectionError();setStartTimeControls(start);$("duration-value").textContent=durationLabel(minutes);$("composer-date").textContent=`${naturalDay(start)} · ${shortDate(start)}`;$("composer-range").textContent=`${time(start)} — ${time(end)}`;$("composer-status").textContent=invalidReason||"Available to reserve";$("composer-summary").classList.toggle("conflict",Boolean(invalidReason));$("selection-error").classList.toggle("hidden",!invalidReason);$("selection-error").textContent=invalidReason;$("confirm-booking").disabled=Boolean(invalidReason);$("duration-down").disabled=minutes<=15;$("duration-up").disabled=minutes>=180;renderCalendar();
}
function openCancel(reservation){state.cancelEvent=reservation;$("cancel-dialog").showModal()}
function renderMyReservations(){const list=$("my-reservations");list.replaceChildren();const mine=state.reservations.filter(r=>r.mine&&sameDay(new Date(r.start_time),state.viewDate));list.classList.toggle("hidden",!mine.length);if(!mine.length)return;const label=document.createElement("span");label.textContent="Your reservations";list.append(label);mine.forEach(reservation=>{const button=document.createElement("button");button.type="button";button.className="reservation-pill";button.textContent=`${time(new Date(reservation.start_time))}–${time(new Date(reservation.end_time))} · Manage`;button.addEventListener("click",()=>openCancel(reservation));list.append(button)})}
function updateBookingLock(){const reservation=activeMine(),locked=Boolean(reservation);$("reservation-limit").classList.toggle("hidden",!locked);document.querySelector(".booking-composer").classList.toggle("locked",locked);["start-time-text","period-am","period-pm"].forEach(id=>$(id).disabled=locked);$("duration-down").disabled=locked||((state.selection.end-state.selection.start)/60000<=15);$("duration-up").disabled=locked||((state.selection.end-state.selection.start)/60000>=180);$("confirm-booking").disabled=locked||Boolean(selectionError());if(locked){$("composer-summary").classList.remove("conflict");$("composer-date").textContent=`Reserved · ${shortDate(new Date(reservation.start_time))}`;$("composer-range").textContent=`${time(new Date(reservation.start_time))} — ${time(new Date(reservation.end_time))}`;$("composer-status").textContent="Your reservation is confirmed";$("selection-error").classList.add("hidden")}}
...
$("confirm-booking").addEventListener("click",()=>{
  ...
  $("confirm-date").textContent=fullDate(start);$("confirm-time").textContent=`${time(start)} — ${time(end)} · ${durationLabel((end-start)/60000)}`;
...
$("keep-reservation").addEventListener("click",()=>$("cancel-dialog").close());$("confirm-cancel").addEventListener("click",async()=>{const button=$("confirm-cancel");busy(button,true,"Cancelling…");try{await api(`/reservations/${state.cancelEvent.id}`,{method:"DELETE"});$("cancel-dialog").close();announce("Reservation cancelled","The time is now available to the group.","success");await loadReservations(true)}catch(error){announce("Cancellation failed",error.message,"error")}finally{busy(button,false)}});
