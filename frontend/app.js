...
const state = {email:"",user:null,reservations:[],viewDate:startOfDay(new Date()),selection:null,confirmSelection:null,refreshTimer:null,provisioningTimer:null,provisioningNoticeAt:0,cancelEvent:null,reservationLimitMinutes:120};
...
function initialSelection(date=state.viewDate){const now=new Date(),start=new Date(date),minutes=Math.min(60,state.reservationLimitMinutes);start.setHours(sameDay(date,now)?now.getHours():9,sameDay(date,now)?now.getMinutes()+1:0,0,0);return{start,end:new Date(start.getTime()+minutes*60*1000)}}
...
function changeDay(date){const oldStart=state.selection?.start;state.viewDate=startOfDay(date);const start=new Date(state.viewDate);start.setHours(oldStart?.getHours()??9,oldStart?.getMinutes()??0,0,0);const duration=state.selection?state.selection.end-state.selection.start:Math.min(60,state.reservationLimitMinutes)*60*1000;state.selection={start,end:new Date(start.getTime()+duration)};updateSelection();document.querySelector(".schedule").scrollIntoView({behavior:"smooth",block:"start"})}
...
function updateBookingLock(){const reservation=activeMine(),adminOnly=Boolean(state.user&&state.user.self_booking===false&&!state.user.is_admin),locked=Boolean(reservation)||adminOnly;$("reservation-limit").classList.toggle("hidden",!reservation);document.querySelector(".booking-composer").classList.toggle("locked",locked);["start-time-text","period-am","period-pm"].forEach(id=>$(id).disabled=locked);$("duration-down").disabled=locked||((state.selection.end-state.selection.start)/60000<=15);$("duration-up").disabled=locked||((state.selection.end-state.selection.start)/60000>=state.reservationLimitMinutes);$("confirm-booking").disabled=locked||Boolean(selectionError());if(adminOnly&&!reservation){$("composer-summary").classList.remove("conflict");$("composer-status").textContent="An administrator must book this GPU. SSH details print on the host.";$("selection-error").classList.add("hidden")}else if(locked){const start=new Date(reservation.start_time),end=new Date(reservation.end_time);$("composer-summary").classList.remove("conflict");$("composer-date").textContent=sameDay(start,end)?`Reserved · ${shortDate(start)}`:`Reserved across midnight · ${compactDay(start)} → ${compactDay(end)}`;$("composer-range").textContent=reservationRange(start,end);$("composer-status").textContent="Your reservation is confirmed";$("selection-error").classList.add("hidden")}}
...
function changeDuration(delta){const current=(state.selection.end-state.selection.start)/60000,next=Math.max(15,Math.min(state.reservationLimitMinutes,current+delta));state.selection.end=new Date(state.selection.start.getTime()+next*60000);updateSelection()}
